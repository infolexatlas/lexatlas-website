...
/**
 * Get the sample PDF path for a given slug with global fallback
 * Server-side only - requires fs access
 */
export function getSamplePdfPath(slug: string): string {
  // Try specific: /public/kits/samples/FRA-USA-sample.pdf
  // Else global:  /public/kits/samples/LEXATLAS-global-sample.pdf
  const fs = require('fs');
  const path = require('path');
  
  const publicDir = path.join(process.cwd(), 'public');
  const iso3 = slug.toUpperCase();
  const specific = path.join(publicDir, 'kits', 'samples', `${iso3}-sample.pdf`);
  const global = path.join(publicDir, 'kits', 'samples', `LEXATLAS-global-sample.pdf`);
  const specificExists = fs.existsSync(specific);
  const chosen = specificExists ? `/kits/samples/${iso3}-sample.pdf` : `/kits/samples/LEXATLAS-global-sample.pdf`;
  return chosen;
}
