  ...
  return (
    <div className="min-h-screen bg-brand-ivory">
      ...
      <JsonLd data={{
        ...
        '@graph': Object.values(kitsDetail).map((p: any) => ({
          '@type': 'Product',
          name: p.slug,
          brand: { '@type': 'Brand', name: 'LexAtlas' },
          sku: `kit-${p.slug}`,
          url: kitPath(p.slug),
          offers: { '@type': 'Offer', priceCurrency: 'EUR', price: (p.priceEUR ?? 0).toFixed(2), availability: 'https://schema.org/InStock', url: kitPath(p.slug) }
        }))
      }} />
    </div>
  )
