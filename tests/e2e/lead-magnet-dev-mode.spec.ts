...
test.describe('Lead Magnet - Development Mode', () => {
  test.skip('should show save-only mode warning when RESEND_API_KEY is missing', async ({
    page,
  }) => {
    ...
    await expect(
      page.locator('text=Thank you! Your lead is saved.')
    ).toBeVisible()

    // Check for development mode warning
    await expect(
      page.locator(
        'text=Email not sent in development (missing RESEND_API_KEY).'
      )
    ).toBeVisible()
  ...
  test('should handle invalid email format', async ({ page }) => {
    ...
    // Find the lead magnet banner
    const leadMagnetBanner = page
      .locator('[data-testid="lead-magnet-banner"]')
      .first()
  ...
  test('should allow getting another sample after success', async ({
    page,
  }) => {
    // Navigate to home page
    await page.goto('/')
