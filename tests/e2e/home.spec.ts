...
test.describe('Home Page', () => {
  test('should load home page successfully', async ({ page }) => {
    ...
    await expect(
      page.getByRole('main').getByRole('link', { name: /Browse Kits/ })
    ).toBeVisible()

    // Check new Phase 3 components
    await expect(
      page.getByText('Trusted by International Couples')
    ).toBeVisible()
  ...
  test('should navigate to kits page', async ({ page }) => {
    ...
    await page
      .getByRole('main')
      .getByRole('link', { name: /Browse Kits/ })
      .click()
  ...
  test('should have working Top Questions accordion', async ({ page }) => {
    ...
    test.skip(
      true,
      'Top Questions section not present in current polished home; tracked to re-introduce later'
    )

    // Check section heading
    await expect(
      page.getByRole('heading', { name: 'Top Questions' })
    ).toBeVisible()

    // Check that FAQ items are present
    await expect(
      page.getByText('What is a LexAtlas Marriage Kit?')
    ).toBeVisible()
    await expect(page.getByText('Does the kit replace a lawyer?')).toBeVisible()
    await expect(
      page.getByText('Are the guides up to date and verified?')
    ).toBeVisible()
    ...
    await expect(
      page.getByRole('link', { name: 'View All FAQs' })
    ).toBeVisible()
  })
  ...
  test('should expand Top Questions accordion items', async ({ page }) => {
    ...
    const firstQuestion = page
      .getByRole('button')
      .filter({ hasText: 'What is a LexAtlas Marriage Kit?' })
