...
test.describe('Kits Page', () => {
  test('should display kits page with bundles and individual kits', async ({ page }) => {
    await page.goto('/kits')
    
    // Check page title and metadata
    await expect(page).toHaveTitle(/Marriage Kits – France & Priority Countries/)
    await expect(page.locator('h1')).toContainText('Marriage Kits')
    
    // Check bundles section
    await expect(page.getByRole('heading', { name: 'Value Bundles' })).toBeVisible()
    await expect(page.getByText('Bundle of 3 Kits')).toBeVisible()
    await expect(page.getByText('Full Pack (10 Kits)')).toBeVisible()
    ...
    await expect(page.getByRole('heading', { name: 'Individual Marriage Kits' })).toBeVisible()
  ...
  test('should display correct EUR pricing on individual kit cards', async ({ page }) => {
    await page.goto('/kits')
  ...
  test('should show bundle savings information', async ({ page }) => {
    ...
    await expect(page.getByText('Save 15% compared to buying separately')).toBeVisible()
  ...
  test('should show Most Popular badge on Bundle of 3', async ({ page }) => {
    ...
    const bundle3Card = page.locator('text=Bundle of 3 Kits').locator('..').locator('..')
  ...
  test('should have responsive grid layout', async ({ page }) => {
    ...
    const kitCards = page.locator('.grid').nth(1).locator('> div')
    await expect(kitCards).toHaveCount(10)
  })
  ...
  test('should navigate to individual kit pages', async ({ page }) => {
    ...
    const firstViewDetailsButton = page.getByRole('link', { name: 'View Details' }).first()
  ...
  test('should show correct SEO metadata', async ({ page }) => {
    ...
    await expect(metaDescription).toHaveAttribute('content', /Download official legal kits for cross-border marriages with France/)
  ...
  test('should have aligned card layouts with equal heights', async ({ page }) => {
    await page.goto('/kits')
    
    // Check that Switzerland card (long title) is visible and doesn't break layout
    const switzerlandCard = page.locator('text=France – Switzerland').locator('..').locator('..').locator('..')
