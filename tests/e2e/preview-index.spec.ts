...
test.describe('Preview Index Page', () => {
  test('should load preview page successfully', async ({ page }) => {
    ...
    // Check page title
    await expect(page).toHaveTitle(/Free Previews/)
    
    // Check main heading
    await expect(page.locator('h1')).toContainText('Free Previews')
    
    // Check description (be more specific)
    await expect(page.locator('p').first()).toContainText('Get a free sample from our cross-border marriage kits')
  })
  ...
  test('should have working global sample download', async ({ page }) => {
    ...
    const downloadButton = page.getByRole('link', { name: /Download global sample/ })
    await expect(downloadButton).toBeVisible()
    
    // Check the href attribute
    await expect(downloadButton).toHaveAttribute('href', '/kits/samples/LEXATLAS-global-sample.pdf')
    
    // Test the download link using fetch instead of page.goto
    const response = await page.request.get('/kits/samples/LEXATLAS-global-sample.pdf')
  ...
  test('should show grid with 10 FRA-X kit cards', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /Preview a France–X kit/ })).toBeVisible()
    
    // Check that there are exactly 10 cards
    const cards = page.locator('[data-testid="kit-card"]')
  ...
  test('should have working lead magnet form', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="lead-magnet-banner"]')).toBeVisible()
  ...
  test('should have proper navigation structure', async ({ page }) => {
    ...
    // Check CTA buttons
    await expect(page.getByRole('link', { name: /Download global sample/ })).toBeVisible()
    await expect(page.getByRole('link', { name: /Get preview by email/ })).toBeVisible()
